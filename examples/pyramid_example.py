"""
Example: Generate and visualize a pyramid mesh.

This demonstrates:
- Pyramid geometry primitive
...
def main():
    """Generate and visualize a pyramid mesh."""
    
    print("=" * 60)
    print("Pyramid Mesh Example")
    print("=" * 60)
    
    # Create a pyramid
    base_size = 1.0
    height = 1.5
    pyramid = Pyramid(base_size, height, center=(0, 0, 0))
    
    print(f"\nPyramid base size: {base_size}")
    print(f"Pyramid height: {height}")
    print(f"Pyramid center: {pyramid.center}")
    
    # Create mesh using trimesh
    print("\nCreating mesh with trimesh...")
    mesher = PythonMesher()
    trimesh_obj = pyramid.to_trimesh()
    ...
    output_file = os.path.join(images_dir, 'pyramid_example.png')
